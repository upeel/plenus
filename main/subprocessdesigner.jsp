<%-- 
    Document   : subprocessdesigner
    Created on : Jul 16, 2014, 2:55:32 PM
    Author     : SOE HTIKE
--%>
...
<%
    ...
    ArrayList<SubProcessHeader> subProcessList = subProcCtrl.getAllSubProcesses();
%>
<html>
    <head>
    ...
    <script type="text/javascript">
        ...
        function init(){
            ...
            root = tree.getRoot();
            tmpNode = new YAHOO.widget.TextNode({ label: "<b>Sub Process List</b>", id:"0" }, root);
            tmpNode.labelStyle="icon-orgunit";
        <%
            for (int i = 0; i < subProcessList.size(); i++) {
                SubProcessHeader subProc = subProcessList.get(i);
        %>
                var tmpSubNode = new YAHOO.widget.TextNode({label:"<%=subProc.getName()%>", id:"<%=subProc.getId()%>"}, tmpNode);
        ...
        %>
                ...
                tree.subscribe("expand", function(node) {
                    if(node.parent.data.id != 0){
                        ...
                        $('#iuser').attr('src', "subprocessdetail.jsp?type="+type+"&subtype="+subtype+"&parentId="+parentId);
            ...
            <table width="980px" height="420px" background="../images/background.png">
                <tr>
                    ...
                    <td class="orgtable" valign="top">
                        <iframe id="iuser" name="iuser" MARGINHEIGHT="0" MARGINWIDTH="0" FRAMEBORDER="0" SRC="subprocessdetail.jsp" width=100% height="420px" ></iframe>
                    </td>
                </tr>
            </table>
        </div>
